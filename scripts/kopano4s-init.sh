...
case "$1" in
	...
	all)
		...
		;;
	help)
		echo "Usage: kopano-init plus reset | refresh | maintenance | no-ports | etc | acl | ssl | mobiles | upgrade | all | help"
		echo "Reset builds a new container from lokal k4s image; using refresh the latest image will be loaded from the docker hub."
